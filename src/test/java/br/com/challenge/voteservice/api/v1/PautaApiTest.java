...
public class PautaApiTest {
    ...
    @Test
    public void getVoteCountWhenPautaNotFound() throws Exception {
        String validBody = new String(Files.readAllBytes(Paths.get("src/test/resources/json/pautaRequest.json")));
        Mockito.doThrow(PautaNotFoundException.class).when(pautaService).getVoteCount(Mockito.anyInt());
        mvc.perform(get("/v1/pautas/1")
                .contentType(MediaType.APPLICATION_JSON)
                .content(validBody))
    ...
    @Test
    public void getVoteCountWhenPautaDoesNotHaveVoteException() throws Exception {
        String validBody = new String(Files.readAllBytes(Paths.get("src/test/resources/json/pautaRequest.json")));
        Mockito.doThrow(PautaDoesNotHaveVoteException.class).when(pautaService).getVoteCount(Mockito.anyInt());
        mvc.perform(get("/v1/pautas/1")
                .contentType(MediaType.APPLICATION_JSON)
                .content(validBody))
                .andExpect(status().is4xxClientError());
